# -*- coding: utf-8 -*-
"""
Bionet Fauna Query -> ArcGIS Online all_species sync (v1.0 - Dr. Petrov Edition)
...
# v1.0: Keep all REQUEST_COLUMNS in final output
...
def fetch_all_species_df(username=None, password=None, page_size=1000, sleep_between_pages=0.2, timeout=60):
    ...
    # v1.0: Case-insensitive sorting by kingdom, class, order, family, scientificName
...
class AllSpeciesSyncTool(object):
    ...
    def getParameterInfo(self):
        ...
        p4 = arcpy.Parameter(
            ...
            parameterType="Required",
            direction="Input")
        p4.value = FORCE_CREATE_IF_MISSING
    ...
    def execute(self, parameters, messages):
        # Use DEFAULT values instead of parameters per requirements
        ...
        petrov_message(f"=== BionetFaunaQuery v{VERSION} Execution Started ===")
        ...
        try:
            if username and password:
                ...
                # v1.0: Try 'home' signin pickup for ArcGIS Pro
                try:
                    gis = GIS('home')
                    petrov_message("Connected via GIS('home') - ArcGIS Pro profile")
                except Exception:
                    gis = GIS(ago_url)
        ...
        if item is None:
            ...
            item_props = {
                "title": target_title, 
                "tags": f"bionet,all_species,automated,v{VERSION}",
        ...
        # v1.0: Tactical 'order' field handling
        attempt_field_rename_order(flc)
        
        # v1.0: Create attribute indexes
        create_attribute_indexes(flc)
        
        # v1.0: Update item metadata with formal legal policy
        update_item_metadata(item, record_count)
        
        # v1.0: Evidence incinerator - delete CSV item if it was created
        ...
        petrov_message(f"=== Sync complete. All v{VERSION} operations executed. ===")
