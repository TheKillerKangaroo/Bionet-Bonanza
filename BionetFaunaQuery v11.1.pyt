...
# ---------------------------------------------------------------------
# CONFIGURATION (hard-coded per request)
# ---------------------------------------------------------------------
# Always use this AGOL URL (ignore parameter)
DEFAULT_AGOL_URL = "https://andersonep.maps.arcgis.com/"

# Always use this target title (ignore parameter)
DEFAULT_TARGET_TITLE = "BIONET_ALL_SPECIES"

# Always create (publish) the hosted table if missing
FORCE_CREATE_IF_MISSING = True
# ---------------------------------------------------------------------
...
def sanitize_text(value, max_length=None):
    """
    Sanitize textual values to remove HTML and control characters which
    can cause AGOL to reject edits with "invalid html content".
    ...
    Returns cleaned string, or None if input is None/empty after cleaning.
    """
    if value is None:
        return None
    try:
...
def fetch_all_species_df(username=None, password=None, page_size=1000, sleep_between_pages=0.2, timeout=60):
    ...
    Robust paging:
      - If the OData response contains @odata.nextLink, follow it.
      - Otherwise fall back to offset paging using $skip/$top.
    """
    ...
    while True:
        ...
        # Build request URL: prefer next_link if we have one, otherwise construct using $skip
        if next_link:
            req_url = next_link
        else:
            ...
            req_url = _build_url(RESOURCE, params)
        ...
        # Extract total count (first page)
        ...
        if total_count and isinstance(total_count, (int, float)) and total_count > 0:
        ...
        # Decide how to page next
        # First check for nextLink in response
        ...
        # If no next_link, use offset ($skip) fallback
        # If page returned fewer rows than page_size, we're done
        if not use_nextlink:
            if not page_values or len(page_values) < page_size:
                break
            skip += page_size
            sleep(sleep_between_pages)
            continue

        # If we had been using nextlink but none provided now, break
        break
...
class AllSpeciesSyncTool(object):
    ...
    def getParameterInfo(self):
        p0 = arcpy.Parameter(
            displayName="AGOL URL (ignored - hardcoded)",
            name="ago_url",
            datatype="GPString",
            parameterType="Required",
            direction="Input")
        p0.value = DEFAULT_AGOL_URL
        ...
        p3 = arcpy.Parameter(
            displayName="Target hosted table title (ignored - hardcoded)",
            name="target_title",
            datatype="GPString",
            parameterType="Required",
            direction="Input")
        p3.value = DEFAULT_TARGET_TITLE

        p4 = arcpy.Parameter(
            displayName="Create if missing? (ignored - always true)",
    ...
    def execute(self, parameters, messages):
        # Parameters (note: AGOL URL and target title are hardcoded above)
        ...
        ago_url = DEFAULT_AGOL_URL
        target_title = DEFAULT_TARGET_TITLE
        create_if_missing = FORCE_CREATE_IF_MISSING
        ...
        try:
            # 1) If explicit credentials provided, use them
            if username and password:
                petrov_message("Using provided AGOL username/password for authentication")
                gis = GIS(ago_url, username, password)
                connection_method = "username/password"
            else:
                # 2) Try to pick up ArcGIS Pro signed-in user via GIS('home')
                connection_method = None
                try:
                    petrov_message("No credentials provided; attempting to use ArcGIS Pro signed-in account via GIS('home')...")
                    gis = GIS("home")
                    connection_method = "GIS('home')"
                except Exception as ex_home:
                    petrov_warning(f"GIS('home') failed: {ex_home}")
                    # 3) Try named profile 'pro' as a common saved profile name
                    try:
                        petrov_message("Attempting to use saved profile 'pro' via GIS(profile='pro')...")
                        gis = GIS(profile="pro")
                        connection_method = "GIS(profile='pro')"
                    except Exception as ex_profile:
                        petrov_warning(f"GIS(profile='pro') failed: {ex_profile}")
                        # 4) Fallback to anonymous connection to provided AGO URL
                        petrov_message("Falling back to anonymous connection GIS(ago_url). Publishing will require authentication.")
                        gis = GIS(ago_url)
                        connection_method = "anonymous"
            # Determine whether we're authenticated by checking gis.users.me
            try:
                me = gis.users.me
            except Exception:
                me = None
            user_display = me.username if me else 'anonymous'
            petrov_message(f"Connected via {connection_method}. Logged in: {user_display}")
        except Exception as e:
        ...
        try:
            me = gis.users.me
            owner = me.username if me else None
        except Exception:
            owner = None
        ...
        if item is None:
            if not create_if_missing:
                ...
                return
            # If we are anonymous (not authenticated) we must not attempt to add/publish
            if owner is None:
                petrov_error("Publishing a new hosted table requires an authenticated AGOL user. "
                             "Please provide AGOL credentials (username/password), sign into ArcGIS Pro, or configure a GIS profile.")
                raise RuntimeError("Authentication required for publishing")
            ...
            # Sanitize text columns before writing CSV to prevent 'invalid html content' errors during publish/append
            text_cols = df_for_csv.select_dtypes(include=['object']).columns.tolist()
            for c in text_cols:
                df_for_csv[c] = df_for_csv[c].apply(lambda v: sanitize_text(v) if pd.notna(v) else v)
            ...
            # Add the CSV as a content item (requires authenticated user)
            try:
                csv_item = gis.content.add(item_properties=item_props, data=csv_path)
            except Exception as e:
                petrov_error(f"Failed to upload CSV item (ensure user is authenticated and has permission): {e}")
                raise
            ...
            # Try truncate (replace existing rows)
            try:
        ...
        for idx, row in df.fillna("").iterrows():
            ...
            for col in df.columns:
                ...
                val = row[col]
                # Sanitize textual values to ensure AGOL will accept them
                if isinstance(val, str):
                    val = sanitize_text(val)
                if isinstance(val, pd.Timestamp):
                    attrs[field_name] = val.isoformat()
                elif isinstance(val, (float, int, bool)) or val is None:
                    # Keep numeric/boolean/None as-is (coerce NaN to None)
                    attrs[field_name] = None if (isinstance(val, float) and math.isnan(val)) else val
                else:
                    # Fallback: coerce to sanitized string
                    try:
                        s = sanitize_text(val)
                        attrs[field_name] = s
                    except Exception:
